...
class Calc:
    ...
    def _state_dep(func):
        ...
        def require_rerun(self):
            if self._rerun:
                raise ValueError(
                    "Need to rerun adrivr before accessing data. Use Calc.run_driver()."
                )
            else:
                return func(self)
    ...
    def __init__(
        ...
        if calc_file is not None:
            ...
            raise ValueError(
                ...
                + ", ".join(kwargs.keys())
                + "\nMissing: "
                + ", ".join([k for k, v in kwargs.items() if v is None])
            )
        else:
            del self.calc_file
    ...
    def set_scan(self, time, duration_min):
        ...
        calc.calc_input.intrvls2min = (
            np.ceil(duration_min / 2 + 0.001) + 1
        )  # Number of 2 min intervals
        self._rerun = True
...
def is_initialized(cb, item):
    ...
    return (
        ...
        or cb == "ut1cm"
        and item in ("centj", "dj1900", "dj2000")  # cut1m.f
        or cb == "axocm"
        and item == "richm"  # caxom.f
        or cb == "wobcm"
        and item == "len_wob_table"  # dinit.f
        or cb == "stacm"
        and item == "nflag"  # dstrt.f
        or cb == "atmcm"
        and item in ("rf", "rtrace", "wetcon")  # catmm.f
